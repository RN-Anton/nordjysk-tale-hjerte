...
# Use nginx's native envsubst template system for runtime env injection.
# Copy index.html as a template; nginx's 20-envsubst-on-templates.sh will
# process it at startup, replacing ${VITE_CLIENT_ID} and ${VITE_TENANT_ID}.
RUN mkdir -p /etc/nginx/templates
COPY --from=builder /app/dist/index.html /etc/nginx/templates/index.html.template
ENV NGINX_ENVSUBST_TEMPLATE_DIR=/etc/nginx/templates
ENV NGINX_ENVSUBST_OUTPUT_DIR=/usr/share/nginx/html
ENV NGINX_ENVSUBST_TEMPLATE_SUFFIX=.template
# Only substitute our specific variables (leave $-signs in JS code alone)
...
RUN apk add --no-cache libcap \
    ...
    && chown -R nginx:nginx /var/cache/nginx /var/log/nginx /etc/nginx/conf.d /etc/nginx/templates /usr/share/nginx/html \
